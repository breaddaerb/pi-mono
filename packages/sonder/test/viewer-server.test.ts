...
describe("viewer server", () => {
	...
	it("serves viewer page and snapshot content", async () => {
		...
		writeFileSync(snapshotPath, "<html><body><h1>Snapshot</h1></body></html>", "utf8");
		...
		try {
			...
			expect(page).toContain("Viewer skeleton");

			const snapshotResponse = await fetch(`${viewer.baseUrl}/viewer/items/item_view/snapshot`);
			expect(snapshotResponse.status).toBe(200);
			expect(await snapshotResponse.text()).toContain("Snapshot");
			...
			expect(await annotationsResponse.text()).toContain("annotations");
		} finally {
			await viewer.close();
			app.close();
		}
